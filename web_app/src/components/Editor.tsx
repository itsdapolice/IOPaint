...
import { Eraser, Eye, Redo, Undo, Expand, Download, Paintbrush } from "lucide-react"
...
export default function Editor(props: EditorProps) {
  ...
  const [
    ...
    handleCanvasEraseMouseDown,
    handleCanvasMouseMove,
    handleCanvasEraseMouseMove,
    undo,
    redo,
    undoDisabled,
    redoDisabled,
    ...
    state.handleCanvasEraseMouseDown,
    state.handleCanvasMouseMove,
    state.handleCanvasEraseMouseMove,
    state.undo,
    state.redo,
    state.undoDisabled(),
    state.redoDisabled(),
  ...
  const curDelLineGroup = useStore((state) => state.editorState.curDelLineGroup)
  ...
  const hadDrawSomething = useCallback(() => {
    return (curLineGroup.length !== 0) || (curDelLineGroup.length !== 0)
  }, [curLineGroup,curDelLineGroup])
  ...
  useEffect(() => {
    ...
    drawLines(context,curDelLineGroup,"#32a852bb")
  }, [
    temporaryMasks,
    extraMasks,
    isOriginalLoaded,
  ...
  const onMouseDrag = (ev: SyntheticEvent) => {
    ...
    if (curLineGroup.length === 0 && curDelLineGroup.length === 0) {
      return
    }
    if (eraseMaskMode) {
      console.log('erase moving')
      handleCanvasEraseMouseMove(mouseXY(ev))
    }
    else{ handleCanvasMouseMove(mouseXY(ev))}
  }
  ...
  const onMouseDown = (ev: SyntheticEvent) => {
    ...
    if (eraseMaskMode) {
      console.log('erase mask mouse down')
      setIsDraging(true)
      handleCanvasEraseMouseDown(mouseXY(ev))
    } else
    {
      setIsDraging(true)
  ...
  const toggleEraseMaskMode = () => {
    toggleEraseMask()
    console.log(
      `Erase Mask Mode: ${eraseMaskMode}`
    )
  }
  ...
  const renderBrushEraseMask = (style: any) => {
    return (
      <div
        className="absolute rounded-[50%] border-[1px] border-[solid] border-[#32a852] pointer-events-none bg-[#32a852bb]"
        style={style}
      />
    )
  }
  ...
  return (
    ...
    >
      ...
      {showBrush &&
        ...
        (interactiveSegState.isInteractiveSeg
          ? renderInteractiveSegCursor()
          : renderBrushEraseMask(getBrushStyle(x, y)))}
      ...
      <div className="fixed flex bottom-5 border px-4 py-2 rounded-[3rem] gap-8 items-center justify-center backdrop-filter backdrop-blur-md bg-background/70">
        ...
        <div className="flex gap-2">
          ...
          <IconButton
            tooltip="Switch to Erase Mask Mode"
            onClick={toggleEraseMaskMode}
          >
            <Paintbrush/>
          </IconButton>
          <IconButton
