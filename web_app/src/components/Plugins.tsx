...
const pluginMap = {
  ...
  [PluginName.MangaTextMask]: {
    IconClass: Slice,
    showName: "Manga Text Mask",
  },
...
const Plugins = () => {
  ...
  const renderGenMaskPlugin = (plugin: PluginInfo) => {
    const { IconClass, showName } = pluginMap[plugin.name as PluginName]
    return (
      <DropdownMenuSub key={plugin.name}>
        <DropdownMenuSubTrigger disabled={disabled}>
  ...
  const renderPlugins = () => {
    return plugins.map((plugin: PluginInfo) => {
      ...
      if (plugin.name === PluginName.MangaTextMask) {
        return renderGenMaskPlugin(plugin)
      }
      return (
        <DropdownMenuItem
          key={plugin.name}
