...
type EditorState = {
  ...
  delLineGroups: LineGroup[]
  lastDelLineGroup: LineGroup
  curDelLineGroup: LineGroup
  ...
  redoCurDelLines: Line[]
  redoDelLineGroups: LineGroup[]
}
...
type AppAction = {
  ...
  handleCanvasEraseMouseDown: (point: Point) => void
  handleCanvasMouseMove: (point: Point) => void
  handleCanvasEraseMouseMove: (point: Point) => void
  ...
  persist(
    immer((set, get) => ({
      ...
      runInpainting: async () => {
        ...
        let maskDelLineGroup: LineGroup = []
        if (useLastLineGroup === true) {
          maskLineGroup = lastLineGroup
          maskImages = prevExtraMasks
        } else {
          maskLineGroup = curLineGroup
          maskDelLineGroup = curDelLineGroup
      ...
      handleCanvasEraseMouseDown: (point: Point) => {
        let lineGroup: LineGroup = []
        const state = get()
        if (state.runMannually()) {
          lineGroup = [...state.editorState.curDelLineGroup]
        }
        lineGroup.push({ size: state.getBrushSize(), pts: [point] })
        set((state) => {
          state.editorState.curDelLineGroup = lineGroup
        })
      },
      ...
      handleCanvasEraseMouseMove: (point: Point) => {
        set((state) => {
          const curDelLineGroup = state.editorState.curDelLineGroup
          if (curDelLineGroup.length) {
            curDelLineGroup[curDelLineGroup.length - 1].pts.push(point)
          }
        })
      },
      ...
      toggleEraseMask: () => {
        set((state) => {
          state.eraseMaskMode = !(get().eraseMaskMode)
        })
      },
      ...
      adjustMask: async (operate: AdjustMaskOperate) => {
        const { imageWidth, imageHeight } = get()
        const { curLineGroup, curDelLineGroup, extraMasks } = get().editorState
